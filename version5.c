...
//void Quita (ListaSockets *lista, int socket)
//{
//	int i=0;
//	int encontrado = 0;
...
void *AtenderCliente (void *socket)
{
	...
	while(terminar==0)
	{
		...
		else if (codigo==9) //envia una frase
		{
			...
			{
				strcpy(respuesta1,"9/");
				write(sock_conn,respuesta1,stlen(respuesta1));
				//Enviar frases a activos
				sprintf(notificacion,"11/%s",frase);
				int i;
				for(i=0; i< listaActivos.num; i++)
					write(listaActivos.sockets[i], notificacion,strlen (notificacion));
					
			}
			
		}		
		 
		else if (codigo==10) // pasarme a activo
			
		{
			pthread_mutex_lock(&mutex);
			PonSocket(&listaActivos,sock_conn);
			pthread_mutex_unlock(&mutex);
			
			
		}
		
		else if (codigo==11) // pasarme a pasivo
		{
			pthread_mutex_lock(&mutex);
