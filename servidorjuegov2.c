...
void *AtenderCliente (void *socket)
{
	...
	while(terminar==0)
	{
		...
		if((codigo!=0) && (codigo!=6) && (codigo!=7))
		{
			...
			//*modificamos la conexión ya que ahora no sera el localhost sino 
			// que sera shiva tambien se modifica el mobre de la base de dato deja de ser
			//juego y pasa a llamarse T3-BBDDjego
		...
		else if(codigo==1) //Registra un usuario
		{
			...
			strcpy (consulta, "INSERT INTO T3_BBDDJuego.jugador VALUES (");
			//concatenamos el id
		...
		else if(codigo==2) //quieren iniciar sesion
		{
			...
			strcpy (consulta,"SELECT password FROM T3_BBDDJuegoo.jugador WHERE jugador.nombre = '"); 
			strcat (consulta, user);
			strcat (consulta,"'");
			// hacemos la consulta 
			err=mysql_query (conn, consulta); 
		...
		{
			strcpy (consulta,"SELECT jugador.nombre FROM T3_BBDDJuego.jugador,T3_BBDDJuego.relacion,T3_BBDDJuego.partida WHERE partida.tiempo_stop = (SELECT MIN(partida.tiempo_stop) FROM Juego.partida) AND partida.id = relacion.idPartida");
			// hacemos la consulta 
			err=mysql_query (conn, consulta); 
			if (err!=0) {
				printf ("Error al consultar datos de la base %u %s\n",
		...
		else if(codigo==4) //Nos da el menor tiempo en ganar del usuario pedido
		{
			...
			strcpy (consulta,"SELECT partida.tiempo_stop FROM T3_BBDDJuego.jugador,T3_BBDDJuego.relacion,T3_BBDDJuego.partida WHERE jugador.nombre = '"); 
			strcat (consulta, user);
		...
		else if (codigo==5) //Nos da el numero de partidas ganadas por el jugador
		{
			int cont;
			//Construimos la consulta
			strcpy (consulta,"SELECT partida.ganador FROM T3_BBDDJuego.partida ");
			// hacemos la consulta 
			err=mysql_query (conn, consulta); 
			if (err!=0) 
			{
...
int main(int argc, char *argv[])
{
	...
	//modificamos el puerto por que se realiza la escucha 
	int puerto =(50059);
