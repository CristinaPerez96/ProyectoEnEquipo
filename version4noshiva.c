...
// Debemos de detallas las caracteristicas que debe de tener nuestro jugador
//jugador dentro de una partida 
typedef struct
	{
	//que necesitamos de un jugador 
	int socket;
	int nombre;
}Jugador;
typedef struct
...
ListaNot Notificacion ;

//Declaraciones "glovales"
...
// relacion del nombre del jugador y su socket correspondiente esto nos servira para enviar la invitacion 
int Socketiniv(ListaConectados *lista, char nombre[20])
	
{
	...
	if(encontrado==1)
	{
		int socket = miLista.conectados[i].socket;
		return socket;
	}
	else
	{
...
//Tomara la posición de un socket dentro de la lista de conectados
int InvitaConectado(char nom[100], int socket, ListaConectados*lista)
{
	int encontrado = 0;
	int i=0;
	...
	strcpy(nom, lista->conectados[i].nombre);
	pthread_mutex_unlock(&mutex);
}

int Confirmacion(ListaConectados *l, char nombre[20], int confirmacion)
{
	int encontrado=0;
	int i=0;
	...
	if(encontrado==1)
	{
		miLista.conectados[i].confirmacion = confirmacion;
		if(confirmacion==1)
		{
			contador1++;
		}
...
//De cuantas personas puede llegar hacer una partida limite 2 por el momento 
int LimiteInv(ListaConectados *lista)
{
	int encontrado=0;
	int i=0;
	while((i<lista->num) && (encontrado==0))
	{
		if(lista->conectados[i].confirmacion == 1)
	...
	{
		int socket = miLista.conectados[i].socket;
		miLista.conectados[i].confirmacion = 2;
		return socket;
	}
	else
	{
...
void *AtenderCliente (void *socket)
{
	...
	while(terminar==0)
	{
		...
		else if (codigo==5) //Nos da el numero de partidas ganadas por el jugador
		{
			...
			// y una columna que contiene el nombre
			char cont1;
			strcpy(respuesta, "5/El número de partidas de user: "); 
			strcat (respuesta, user);
		...
		if(codigo==6) //Recibimos las invitaciones i enviamos a los invitados
		{
			char emisor[20];
			char receptor[20];
			strcpy (emisor,Notificacion.mensajes[0].respuesta);
			strcpy(receptor,Notificacion.mensajes[1].respuesta);
			sprintf(respuesta,"7/7/%s", receptor);
			write (sock_conn,respuesta, strlen(respuesta));
		}
		if(codigo==7) //Respuesta de la Invitacion
		{
			char emisor[20];
			char nombredelgrid[20];
			strcpy(nombredelgrid,Notificacion.mensajes[0].respuesta);
			strcpy(emisor, Notificacion.mensajes[1].respuesta);
			int a = atoi(Notificacion.mensajes[2].respuesta);
			char *emisor1 = strtok( emisor, "\0");
			int confirmacion=1;
			int b = Confirmacion(&miLista, emisor1,confirmacion);
			contador1=contador1-1;
			int c=Socketiniv(&miLista, nombredelgrid);
			char *peticion=strtok(respuesta,"\0");
			sprintf(respuesta,"7/%s/%s/%d",nombredelgrid,emisor1,a);
			char *notificacion1 = strtok( respuesta, "\0");
			write (c,notificacion1, strlen(notificacion1));
		}
		if(codigo==8) //confirmar invitacion
		{
			int confirmacion; 
			char receptor[20];
			int numerodegrid;
			confirmacion = atoi(Notificacion.mensajes[1].respuesta);
			strcpy(receptor, Notificacion.mensajes[0].respuesta);
			numerodegrid = atoi(Notificacion.mensajes[2].respuesta);
			int a = Confirmacion(&miLista, receptor,confirmacion);
			if(confirmacion==1)
			{
				if (a==0)
				{
					printf("Invitacion Aceptada");
					if(contador1==numerodegrid)
					{
						int i=0;
						while(i<contador1+1)
						{
				...
				{
					printf(" Invitacion no aceptada");
				}
			}
			else
			{
				int i;
				if (contador2==numerodegrid)
				{
					for(i=0;i<contador2+1;i++)
					{
						int j =
							LimiteInv(&miLista);
						sprintf(respuesta, "8/");
